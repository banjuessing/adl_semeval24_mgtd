...
from transformers import AutoTokenizer, AutoModel, GPT2Tokenizer, GPT2Model, XLNetTokenizer, XLNetModel
...
class MGTDDataset(Dataset):
    def __init__(self, txts, lbls, 
        ...
        self.sep_token = ['<sep>'] if 'xlnet' in model_name else ['<|endoftext|>']
...
class TransformerModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        hiddens = self.mean_pooling(raw_outputs, inputs['attention_mask'])
        cls_feats = hiddens
        if self.loss_func == "dualcl":
            label_feats = raw_outputs.last_hidden_state[:, :self.num_labels, :]
